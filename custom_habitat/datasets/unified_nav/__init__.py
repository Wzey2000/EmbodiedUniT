#!/usr/bin/env python3

# Copyright (c) Facebook, Inc. and its affiliates.
# This source code is licensed under the MIT license found in the
# LICENSE file in the root directory of this source tree.

from custom_habitat.core.dataset import Dataset
from custom_habitat.core.registry import registry

from .unified_nav_dataset import (  # noqa: F401
            UnifiedDatasetV1
        )


# def _try_register_unified_nav_dataset():
#     try:
#         from custom_habitat.datasets.unified_nav.unified_dataset import (  # noqa: F401 isort:skip
#             UnifiedDatasetV1,
#         )
#     except ImportError as e:
#         r2r_vln_import_error = e

#         @registry.register_dataset(name="R2RVLN-v1")
#         class R2RDatasetImportError(Dataset):
#             def __init__(self, *args, **kwargs):
#                 raise r2r_vln_import_error
