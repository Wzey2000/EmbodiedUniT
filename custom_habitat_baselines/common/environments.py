...
from custom_habitat.utils.visualizations.utils import observations_to_image, append_text_to_image
from gym.spaces.dict import Dict as SpaceDict
from gym.spaces.box import Box
from gym.spaces.discrete import Discrete

from custom_habitat import Config, Dataset, RLEnv
...
class NavRLEnv(RLEnv):
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        ...
        self._success_measure_name = 'success'
        ...
        self.SUCCESS_REWARD = config.RL.SUCCESS_REWARD
        self.COLLISION_REWARD = config.RL.COLLISION_REWARD
        self.SLACK_REWARD = config.RL.SLACK_REWARD
    ...
    def get_reward(self, observations):
        ...
        reward += max(0, self._previous_measure - current_measure) * 0.2
    ...
    def get_done(self, observations):
        ...
        if self._env.episode_over or self._episode_success() or np.isinf(self._env.get_metrics()['distance_to_goal']):
            done = True
        return done
...
For ImageNav
"""
@baseline_registry.register_env(name='SearchEnv')
...
@baseline_registry.register_env(name='MultiSearchEnv')
