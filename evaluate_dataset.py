...
from env_utils.make_env_utils import add_panoramic_camera, add_camera
import habitat
...
def get_eval_config(args):
    ...
    if use_panorama:
        ...
        config.TASK_CONFIG = add_camera(config.TASK_CONFIG, normalize_depth=True)
    ...
    if args.stop:
        config.ACTION_DIM = 6
        config.TASK_CONFIG.TASK.POSSIBLE_ACTIONS = ["STOP", "MOVE_FORWARD", "TURN_LEFT", "TURN_RIGHT", "LOOK_UP", "LOOK_DOWN"]
    else:
        config.ACTION_DIM = 5
        config.TASK_CONFIG.TASK.POSSIBLE_ACTIONS = ["MOVE_FORWARD", "TURN_LEFT", "TURN_RIGHT", "LOOK_UP", "LOOK_DOWN"]
...
def evaluate(eval_config, ckpt):
    ...
    if RANDOM_AGENT:
        ...
        eval_info += 'Stop action: {}\n'.format('True' if eval_config.ACTION_DIM==6 else 'False')
        eval_info += '===========================================\n'
    ...
    if torch.cuda.device_count() > 0:
        device = torch.device("cuda:"+str(eval_config.TORCH_GPU_ID[0]))
        runner.to(device)
    ...
    if runner.need_env_wrapper:
        ...
        env.habitat_env.get_next_episode_search = next_episode
