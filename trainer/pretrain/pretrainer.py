...
import cv2
import copy
from trainer.pretrain.pretrain_wrapper import PretrainWrapper
...
class UpSampleLayer(nn.Module):
    def __init__(self, in_ch, out_ch) -> None:
        ...
        self.upsample = nn.Sequential(
            nn.ConvTranspose2d(in_channels=out_ch*2, out_channels=out_ch, kernel_size=3, stride=2, output_padding=1),
            nn.BatchNorm2d(out_ch*2),
            nn.ReLU()
        )
...
class UNetDecoder(nn.Module):
    def __init__(self, cfg) -> None:
        ...
        # 512 -> 256 -> 128 - > 3+1
        chs = [256, 128, 64, 4]
        self.u1 = UpSampleLayer(cfg.features.visual_feature_dim, chs[0])
        self.u2 = UpSampleLayer(chs[0], chs[1])
        self.u3 = UpSampleLayer(chs[1], chs[2])
        self.u4 = UpSampleLayer(chs[2], chs[3])
        self.output = nn.Sequential(
            nn.Conv2d(chs[3], chs[3], 3, 1, 1),
            nn.Sigmoid()
        )
...
class Pretrainer(nn.Module):
    def __init__(self, cfg, perception):
        ...
        self.env_wrapper = PretrainWrapper(cfg, cfg.BC.batch_size)
        ...
        self.optim = optim.Adam(
            list(filter(lambda p: p.requires_grad,self.perception.parameters())),
            lr=cfg.BC.lr
        )
        ...
        if self.forgetting_coef:
            print("[pretrainer] Using Expire-span forgetting mechanism\n")
    ...
    def save(self,file_name=None, env_global_node=None, epoch=0, step=0):
        if file_name is not None:
            ...
            save_dict['state_dict'] = self.perception.state_dict()
    ...
    def forward(self, batch, env_global_node, train=True):
        ...
        for t in range(T):
            ...
            curr_context, goal_context, new_env_global_node, _ = self.perception(
                obs_t,
                env_global_node
            )
            ...
            try:
                if not (gt_act == -100).all():
                    ...
                    if self.forgetting_coef != 0:
                        ...
                        mask, remaining_span, _ = self.perception.get_memory_span()
